...
function ControlPanel() {
  ...
  const [qrModalType, setQrModalType] = useState('join'); // 'join' or 'display'
  ...
  const [isApproved, setIsApproved] = useState(true); // Default true so no flash before load
  ...
  const handleShowQR = async (type = 'join') => {
    try {
      const endpoint = type === 'display'
        ? `/${businessId}/appointments/qr?type=display`
        : `/${businessId}/appointments/qr`;
      ...
      <div className="max-w-6xl mx-auto space-y-6 relative">
            ...
            <div className="bg-slate-800 rounded-3xl p-8 max-w-sm w-full border border-slate-700 shadow-2xl scale-in-center">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-white">
                  {qrModalType === 'display' ? 'Live Queue Display QR' : 'Booking Join QR'}
                </h3>
              ...
              <div className="bg-white p-6 rounded-2xl mb-6 shadow-inner">
                {qrData && <img src={qrData.qrCode} alt="QR Code" className="w-full" />}
              </div>
              <p className="text-slate-400 text-sm text-center mb-6 px-4">
                {qrModalType === 'display'
                  ? 'Show this on a screen for patients to view the live queue status.'
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <button
              onClick={() => handleShowQR('display')}
              className="flex-1 md:flex-none py-2.5 px-4 rounded-lg bg-emerald-600/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-600/20 hover:text-emerald-300 font-bold text-sm transition-all flex items-center justify-center gap-2"
            >
        ...
        {/* Tab Content wrapped in conditional to disable usage if unapproved */}
        <div style={{ opacity: (!isApproved || trialExpired) ? 0.3 : 1, pointerEvents: (!isApproved || trialExpired) ? 'none' : 'auto' }}>
          {activeTab === 'queue' ? (
